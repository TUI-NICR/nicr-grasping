...
def multivariate_gaussian(pos: np.ndarray, mu: np.ndarray, Sigma: np.ndarray) -> np.ndarray:
    ...
    return np.exp(-fac / 2) / N  # type: ignore


def draw_gauss(angle: float, width: float, length: float, center: np.ndarray, shape: Tuple,
               points: Optional[np.ndarray] = None, scaling: float = 1.0, fast_mode: bool = True) -> np.ndarray:
