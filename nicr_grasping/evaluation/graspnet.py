...
def run_predictor(predictor: "torch.engine.DefaultPredictor",
    ...
    def _save_output(output, filename, vmin=None):
        if vmin is None:
            vmin = output.min()
        f, ax = plt.subplots(1, 1, figsize=(10, 5))
        shownimage = ax.imshow(output, cmap=plt.get_cmap('Reds'), vmin=vmin)
        ax.axis('off')
        f.colorbar(shownimage)
        f.tight_layout()
        f.savefig(filename)
        plt.close(f)
    ...
    with torch.no_grad():
        for scene_idx, scene_id in enumerate(tqdm.tqdm(scene_ids, total=len(scene_ids), desc='Iterating scenes')):
            ...
            for ann_id in tqdm.tqdm(range(256), total=256, desc='Iterating samples', leave=False):
                ...
                for key, out in pred[0].items():
                    if isinstance(out, np.ndarray):
                        _save_output(out, current_save_dir + f'_{key}.png')
                    elif isinstance(out, dict):
                        for extra_key, extra_out in out.items():
                            _save_output(extra_out, current_save_dir + f'_{key}_{extra_key}.png')

                        if key in ['aleatoric', 'epistemic']:
                            _save_output(np.sum(list(out.values()), axis=0), current_save_dir + f'_{key}_sum.png', vmin=0)
